...
var traceTick = acorn.parse('$_trace_probe(file, name, startLine, startColumn, endLine, endColumn)');
var traceFunction = acorn.parse('$_trace_Function(file, name, startLine, startColumn, endLine, endColumn)');
...
var visitors = {
  FunctionDeclaration: {
      ...
      node.body.body.unshift(makeTraceFunction(file, node.loc, node.id && node.id.name));
